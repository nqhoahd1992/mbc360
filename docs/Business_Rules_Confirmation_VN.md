...
| **F12** | A3 | Kết nối Cosmetri — *gần như đã giải quyết xong nhờ tài liệu API nhận được*: cơ chế đăng nhập/cấp token đã rõ, nguyên liệu có kèm **tên nhà cung cấp**, và trường "code" của nguyên liệu là **số lô (Batch No.)** — nên danh tính nguyên liệu để đối chiếu (INCI/CAS) lấy từ dữ liệu compliance của Cosmetri. Đã chốt (16/07/2026): dữ liệu Cosmetri không cung cấp (chi tiết nhà cung cấp ngoài tên, link tài liệu SDS/CoA/TDS) thì **nhập tay**; nguyên liệu mới theo quy trình **change request trên Power Apps → phê duyệt → nhập vào Cosmetri**, MBc360 gắn link tới ứng dụng Power Apps khi nguyên liệu chưa có để chọn. **Câu hỏi duy nhất còn lại: phạm vi compliance của Cosmetri có bao phủ ASEAN/Việt Nam không (tài liệu chỉ thấy ví dụ EU/UK/US)?** |
