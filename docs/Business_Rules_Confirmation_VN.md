...
| # | Liên quan | Câu hỏi |
|---|---|---|
| **F1** | C7, B1 | Định nghĩa theo từng gate của "required sign-offs" và "mandatory evidence": với mỗi gate (đặc biệt Gate 07 và 10), chính xác sign-off nào và register/bằng chứng nào ở trạng thái nào là điều kiện pass (ví dụ: Gate 07 ⇐ Formulation_Safety "Final safety release" = Completed VÀ Prohibited_Ingredients không còn dòng "REVIEW"/"Prohibited - remove")? Từng mục là chặn cứng hay cảnh báo mềm? Áp cho cả 37 register hay chỉ nhóm safety-critical? *Khung `gateBlockers()` trong demo đã sẵn sàng enforce danh sách này dạng chặn cứng ngay khi được chốt.* |
| **F2** | C1 | Chọn **"Infant 0+"** (sản phẩm thuần cho em bé, không chọn mục maternal) có kích hoạt Skincare for Two không? Câu trả lời hiện chỉ liệt kê Pregnancy/Breastfeeding/Postpartum. *Giả định tạm của demo: chỉ 3 lựa chọn đó kích hoạt chặn cứng (sửa một dòng code khi được xác nhận).* |
...
| **F6** | A4, C2 | **Ma trận vai trò/quyền** cụ thể (vai trò × gate/section/register × đóng góp/phê duyệt/ký). Dữ liệu user + phòng ban lấy từ đâu (SSO/AD)? Ủy quyền khi vắng mặt? Chuẩn chữ ký điện tử cho "electronic approval history" (độ sâu audit-trail, theo kiểu 21 CFR Part 11 hay nhẹ hơn)? *Giả định tạm của demo: selector "View as role" suy quyền từ Primary owner của gate / department của phase — hạn chế gate decision, "Approved by" của phase và market approvals; các trường bằng chứng vẫn mở.* |
