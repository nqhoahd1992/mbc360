...
export const PROJECT_INCLUDE = {
  ...
  costing: true,
  postLaunchReviews: { orderBy: [{ market: 'asc' }, { dueDate: 'asc' }] },
  supersessionDecisions: { orderBy: [{ version: 'asc' }, { market: 'asc' }] },
  gateSignOffs: {
    orderBy: [{ gateId: 'asc' }, { market: 'asc' }, { role: 'asc' }],
...
export function toProjectData(
  ...
  return {
    ...
    marketTracks: p.marketTracks.map((t) => ({
      ...
      actualLaunchDate: dateOnly(t.actualLaunchDate),
      withdrawnDate: dateOnly(t.withdrawnDate),
      withdrawnReason: opt(t.withdrawnReason),
    })),
    postLaunchReviews: p.postLaunchReviews.map((r) => ({
      id: r.id,
      market: r.market,
      milestone: r.milestone,
      dueDate: dateOnly(r.dueDate) ?? '',
      completedDate: dateOnly(r.completedDate),
    ...
    gateSignOffs: p.gateSignOffs.map((g) => ({
      ...
      ...(g.signedByUserId ? { signedByUserId: g.signedByUserId } : {}),
      name: opt(g.name),
      initials: opt(g.initials),
      ...(g.signedAt ? { signedAt: g.signedAt.toISOString() } : {}),
      roleAtSigning: opt(g.roleAtSigning),
      decision: opt(g.decision) as GateSignOff['decision'],
      comment: opt(g.comment),
      signatureImage: opt(g.signatureImage),
      ...(g.signatureVerifiedAt ? { signatureVerifiedAt: g.signatureVerifiedAt.toISOString() } : {}),
      ...(g.snapshot ? { snapshot: g.snapshot as unknown as GateEvidenceSnapshot } : {}),
    })),
